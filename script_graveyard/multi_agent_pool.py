...
def multi_agent_loop(env, mode, train_ego, ego_version, npc_version, ego_agent, npc_agent, env_config, args, device, trajectory_path, record_video=False, use_pbar=True, ego_pool=None, npc_pool=None):
    if use_pbar:
    ...
    if args.track:
        if wandb.run is not None:
            wandb.finish()
            run = initialize_logging(args, ego_version, npc_version, train_ego, npc_pool_size=npc_pool.size if npc_pool else None, ego_pool_size=ego_pool.size if ego_pool else None)
        else:
            run = initialize_logging(args, ego_version, npc_version, train_ego, npc_pool_size=npc_pool.size if npc_pool else None, ego_pool_size=ego_pool.size if ego_pool else None)

    num_crashes, episode_rewards, duration, episode_speed, ep_rew_total, ep_len_total, ep_speed_total = [], np.zeros(args.num_envs), np.zeros(args.num_envs), np.zeros(args.num_envs), np.zeros(0), np.zeros(0), np.zeros(0)
            ...
            for worker in range(args.num_envs):
                if terminated[worker]:
                    if mode == 'train_ego' or mode == 'eval':
                        ...
                        npc_agent.trajectory_store.add(worker, Transition(npc_state[worker].cpu().numpy(), npc_action[worker].cpu().numpy(), None, reward[worker].cpu().numpy()), int_frames)
                else:
                    if mode == 'train_ego' or mode == 'eval':
                        ...
                        npc_agent.trajectory_store.add(worker, Transition(npc_state[worker].cpu().numpy(), npc_action[worker].cpu().numpy(), obs[1][worker].flatten(), reward[worker].cpu().numpy()), int_frames)
    ...
    if mode == 'train_ego':
        ego_agent.save_model(path=f"E{ego_version}_V{npc_version}_TrainEgo_{str(True)}.pth")
    elif mode == 'train_npc':
        npc_agent.save_model(path=f"E{ego_version}_V{npc_version}_TrainEgo_{str(False)}.pth")
