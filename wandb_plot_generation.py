...
training_runs = api.runs("amar-research/safetyh",
                 filters={"config.model_sampling": "prioritized",
                          "config.eval": False,
...
for i in tqdm(range(1,max_ego_version+1), desc="Concatenating NPC ELOs"):
    ...
    for idx, scan in tqdm(enumerate(history), leave=False, desc=f"Slicing from {start_idx} to {start_idx+episodes}", total=episodes):
