...
class CrashResetWrapper(Wrapper):
    ...
    def _reset(self) -> None:
        ...
        self.road = Road(
            network=RoadNetwork.straight_road_network(lane_count, speed_limit=30),
            np_random=self.get_wrapper_attr("np_random"),
            record_history=show_traj,
        )

        # Create Road Vehicles

        other_vehicles_type = utils.class_from_path(
            self.unwrapped.config["other_vehicles_type"]
        )

        spawn_configs = self.unwrapped.config["spawn_configs"]
        spawn_config = self.get_wrapper_attr("np_random").choice(spawn_configs)
        spawn_distance = self.get_wrapper_attr("np_random").normal(
            self.unwrapped.config["mean_distance"],
            self.unwrapped.config["mean_distance"] / 10,
        )
        starting_vel = self.unwrapped.config["initial_speed"] + self.get_wrapper_attr(
            "np_random"
        ).normal(self.unwrapped.config["mean_delta_v"], 5)
        ...
        for _ in range(self.unwrapped.config["ego_vehicles"]):
            ...
            if spawn_config == "behind_left":
                ...
                lane1 = self.get_wrapper_attr("np_random").choice(
                    self.road.network.graph["0"]["1"]
                )
                vehicle = self.action_type.vehicle_class(
                    road=self.road,
                    position=lane1.position(0, 0),
...
class CrashRewardWrapper(RewardWrapper):
    ...
    def reward(self, reward):
        ...
        if abs(dvx) < self.unwrapped.config["tolerance"]:
            ...
            try:
                r_x = (
                    1.0 / (1.0 + math.exp(-4 - 0.1 * ttc_x))
                    if ttc_x <= 0
                    else -1.0 / (1.0 + math.exp(4 - 0.1 * ttc_x))
                )
        ...
        if abs(dvy) < self.unwrapped.config["tolerance"]:
            ...
            try:
                r_y = (
                    1.0 / (1.0 + math.exp(-4 - 0.1 * ttc_y))
                    if ttc_y <= 0
                    else -1.0 / (1.0 + math.exp(4 - 0.1 * ttc_y))
                )
            except OverflowError:
                r_y = 0.0
