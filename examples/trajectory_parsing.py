...
def plot_trajectories(
    episode_array, frame_rate, fig, axes, time_buffer, episode_num, video_path
):
    action_string = ""
    ...
    axes[1].set_ylim(
        min(
            episode_array[:, 3].min(),
            episode_array[:, 3].min() + episode_array[:, 8].min(),
        ),
        max(
            episode_array[:, 3].max(),
            episode_array[:, 3].max() + episode_array[:, 8].max(),
        ),
    )
    ...
    with writer.saving(
        fig, os.path.join(video_path, config_string, f"{episode_num}.mp4"), dpi=100
    ):
        for i in range(episode_array.shape[0]):
            ...
            ego_anchor = (
                ego_centroid[0] - 2.5 * np.cos(ego_heading),
                ego_centroid[1] - 1.0 * np.cos(ego_heading),
            )
            ego_bbox = patches.Rectangle(
                xy=ego_anchor,
                width=5,
                height=2,
                angle=np.rad2deg(ego_heading),
                fill=False,
                edgecolor="b",
            )
            ...
            npc_anchor = (
                npc_centroid[0] - 2.5 * np.cos(npc_heading),
                npc_centroid[1] - 1.0 * np.cos(npc_heading),
            )
            npc_bbox = patches.Rectangle(
                xy=npc_anchor,
                width=5,
                height=2,
                angle=np.rad2deg(npc_heading),
                fill=False,
                edgecolor="r",
            )
            ...
            axes[0].annotate(
                f"EGO: {ego_vx:.2f}",
                xy=ego_anchor,
                xytext=(ego_anchor[0] + 1, ego_anchor[1] + 1),
            )
            axes[0].annotate(
                f"NPC: {npc_vx:.2f}",
                xy=npc_anchor,
                xytext=(npc_anchor[0] + 1, npc_anchor[1] + 1),
            )
            ...
            if i < vel_plot_data.shape[0] - 1:
                ...
                axes[1].scatter(
                    i,
                    vel_plot_data[i, 3] + vel_plot_data[i, 8],
                    label="NPC",
                    color="red",
                )
...
def plot_traj_history(
    ...
    add_bounding_boxes(
        ego_x, ego_y, ego_heading - 90, ego_vx, ego_size, "green", "Ego Vehicle", "Ego"
    )

    # Add NPC vehicle bounding boxes
    add_bounding_boxes(
        npc_x, npc_y, npc_heading - 90, npc_vx, npc_size, "blue", "NPC Vehicle", "NPC"
    )
