...
for ind in df.index:
    ...
    if isinstance(summary_entry_dict, dict) and isinstance(config_entry_dict, dict):
        ...
        if (
            "eval" in config_keys
            and "model_sampling" in config_keys
            and "adjustable_k" in config_keys
        ):
            ...
            if config_entry_dict["eval"] and (
                "prio_all_5_cycle_eval" in tag_entry_list
            ):
                timestamps.append(summary_entry_dict["_timestamp"])
                ego_v = config_entry_dict["ego_version"]
                npc_v = config_entry_dict["npc_version"]
            ...
            if (
                config_entry_dict["eval"]
                and config_entry_dict["model_sampling"] == "uniform"
                and ("uniform_all_5_cycle_eval" in tag_entry_list)
            ):
                timestamps.append(summary_entry_dict["_timestamp"])
                ego_v = config_entry_dict["ego_version"]
                npc_v = config_entry_dict["npc_version"]
            ...
            if config_entry_dict["eval"] and (
                "local_all_5_cycle_eval" in tag_entry_list
            ):
                timestamps.append(summary_entry_dict["_timestamp"])
                ego_v = config_entry_dict["ego_version"]
                npc_v = config_entry_dict["npc_version"]
...
for i in range(0, max_npc_version + 1):
    plt.plot(
        range(0, max_ego_version + 1),
        eval_table_sequential[:, i],
        label=f"V{i + 1}",
        linestyle="--",
        marker="o",
    )
...
plt.plot(
    range(0, max_ego_version + 1),
    ego_avg_crash_rate_celo,
    label="Ego Version Prioritized",
    linestyle="--",
    marker="o",
)
plt.plot(
    range(0, max_ego_version + 1),
    ego_avg_crash_rate_uniform,
    label="Ego Version Uniform",
    linestyle="--",
    marker="o",
)
plt.xlabel("Version")
...
plt.plot(
    range(1, max_npc_version + 2),
    npc_avg_crash_rate_celo,
    label="NPC Version Prioritized",
    linestyle="--",
    marker="o",
)
plt.plot(
    range(1, max_npc_version + 2),
    npc_avg_crash_rate_uniform,
    label="NPC Version Uniform",
    linestyle="--",
    marker="o",
)
plt.xlabel("Version")
...
eval_table_sequential = np.append(
    eval_table_sequential, np.mean(eval_table_sequential, axis=0).reshape(1, -1), axis=0
)
eval_table_sequential = np.append(
    eval_table_sequential, np.mean(eval_table_sequential, axis=1).reshape(-1, 1), axis=1
)

eval_table_continuous = np.append(
    eval_table_continuous, np.mean(eval_table_continuous, axis=0).reshape(1, -1), axis=0
)
eval_table_continuous = np.append(
    eval_table_continuous, np.mean(eval_table_continuous, axis=1).reshape(-1, 1), axis=1
)

eval_table_uniform = np.append(
    eval_table_uniform, np.mean(eval_table_uniform, axis=0).reshape(1, -1), axis=0
)
eval_table_uniform = np.append(
    eval_table_uniform, np.mean(eval_table_uniform, axis=1).reshape(-1, 1), axis=1
)
...
for i in range(eval_table_sequential.shape[0]):
    for j in range(eval_table_sequential.shape[1]):
        text = ax.text(
            j,
            i,
            round(eval_table_sequential[i, j], 2),
            ha="center",
            va="center",
            color="w",
        )
...
# Add label for mean
ax.text(
    eval_table_sequential.shape[1] - 1,
    eval_table_sequential.shape[0] - 1,
    "Mean",
    ha="center",
    va="center",
    color="r",
...
for i in range(eval_table_continuous.shape[0]):
    for j in range(eval_table_continuous.shape[1]):
        text = ax.text(
            j,
            i,
            round(eval_table_continuous[i, j], 2),
            ha="center",
            va="center",
            color="w",
        )
...
# Add label for mean
ax.text(
    eval_table_continuous.shape[1] - 1,
    eval_table_continuous.shape[0] - 1,
    "Mean",
    ha="center",
    va="center",
    color="r",
...
for i in range(eval_table_uniform.shape[0]):
    for j in range(eval_table_uniform.shape[1]):
        text = ax.text(
            j,
            i,
            round(eval_table_uniform[i, j], 2),
            ha="center",
            va="center",
            color="w",
        )
...
# Add label for mean
ax.text(
    eval_table_uniform.shape[1] - 1,
    eval_table_uniform.shape[0] - 1,
    "Mean",
    ha="center",
    va="center",
    color="r",
