...
for line in tqdm(lines[1:]):
    try:
        ...
        episodes[episode]["action"].append(
            split_line[3]
            .split(":")[-1]
            .replace("tensor", "")
            .replace("]", "")
        ...
        episodes[episode]["reward"].append(
            split_line[4]
            .split(":")[-1]
            .replace("tensor", "")
            .replace("]", "")
        ...
        episodes[episode]["info"].append(
            eval(
                ",".join(split_line[6:])[5:].replace("inf", "0.0").replace("nan", "0.0")
            )
        )
    except:
        print("Error in line:")
