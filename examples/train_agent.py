...
def train_agent(config: dict):
    ...
    gym_config["observation"]["observation_config"]["frame_stack"] = config.get(
        "frame_stack", 1
    )
...
def test_scenarios(config: dict):
    """
    Args:
        env: The environment.
    ...
    for scenario in scenarios:
        ...
        if config.get("track", False):
            ...
            run = initialize_logging(
                config, train_ego=None, npc_pool_size=None, ego_pool_size=None
            )
        ...
        for ep_num in tqdm(range(total_episodes)):
            ...
            obs, info = env.reset()
            ego_state, npc_state = helpers.unpack_states(
                obs, ego_agent, device, vs_mobil
            )
            ...
            while not (done or truncated):
                ...
                if vs_mobil is False:
                    ego_action = (
                        torch.squeeze(ego_agent.predict(ego_state))
                        .view(1, 1)
                        .cpu()
                        .numpy()
                    )
                ...
                helpers.populate_stats(
                    info, ego_agent, ego_state, npc_state, reward, episode_statistics
                )
...
def train_scenarios(config: dict, use_pbar: bool = True):
    """
    Args:
        env: The environment.
        ego_agent (DQN_Agent): The ego agent (multi-agent).
    ...
    while t_step < total_timesteps:
        ...
        next_state, npc_state = helpers.obs_to_state(obs, ego_agent, ego_agent, device)
...
def train_vs_mobil(config: dict, use_pbar: bool = True):
    """
    Args:
        env: The environment.
        ego_agent (DQN_Agent): The ego agent (multi-agent).
